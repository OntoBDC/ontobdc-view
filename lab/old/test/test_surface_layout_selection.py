...
from ontobdc.shared.adapter.surface.selector import (
    DefaultSurfaceLayoutSelector,
)
...
def test_selects_candidate_covering_1_to_4_at_4_columns():
    match = DefaultSurfaceLayoutSelector.select_default_layout(SurfaceCapacity(columns=4, rows=12), TIERED_CANDIDATES)
    assert match.iri == "urn:layout:compact"


def test_selects_candidate_covering_5_to_8_at_5_columns():
    match = DefaultSurfaceLayoutSelector.select_default_layout(SurfaceCapacity(columns=5, rows=12), TIERED_CANDIDATES)
    assert match.iri == "urn:layout:regular"


def test_selects_unbounded_lower_9_candidate_at_9_columns():
    match = DefaultSurfaceLayoutSelector.select_default_layout(SurfaceCapacity(columns=9, rows=12), TIERED_CANDIDATES)
    assert match.iri == "urn:layout:wide"


def test_missing_max_bound_is_unbounded_above():
    match = DefaultSurfaceLayoutSelector.select_default_layout(SurfaceCapacity(columns=999, rows=12), TIERED_CANDIDATES)
    assert match.iri == "urn:layout:wide"


def test_missing_min_bound_is_unbounded_below():
    candidate = _layout("urn:layout:any-below-5", max_available_columns=4)
    match = DefaultSurfaceLayoutSelector.select_default_layout(SurfaceCapacity(columns=1, rows=12), [candidate])
...
def test_row_constraints_are_honored_when_present():
    ...
    assert DefaultSurfaceLayoutSelector.select_default_layout(SurfaceCapacity(columns=1, rows=12), [tall, short]).iri == "urn:layout:tall"
    assert DefaultSurfaceLayoutSelector.select_default_layout(SurfaceCapacity(columns=1, rows=5), [tall, short]).iri == "urn:layout:short"
...
def test_higher_layout_priority_wins_on_overlapping_ranges():
    ...
    match = DefaultSurfaceLayoutSelector.select_default_layout(SurfaceCapacity(columns=6, rows=12), [low, high])
...
def test_specificity_resolves_equal_priority_overlaps():
    ...
    match = DefaultSurfaceLayoutSelector.select_default_layout(SurfaceCapacity(columns=5, rows=12), [broad, narrow])
...
def test_iri_lexical_order_is_final_tiebreaker():
    ...
    match = DefaultSurfaceLayoutSelector.select_default_layout(SurfaceCapacity(columns=5, rows=12), [b, a])
...
def test_contradictory_candidate_is_rejected_rather_than_selected():
    ...
    match = DefaultSurfaceLayoutSelector.select_default_layout(SurfaceCapacity(columns=5, rows=12), [contradictory])
    assert match is None


def test_no_matching_candidate_returns_none():
    match = DefaultSurfaceLayoutSelector.select_default_layout(SurfaceCapacity(columns=1, rows=1), TIERED_CANDIDATES[1:])
    assert match is None
