...
class WorkStreamScriptAdapter(WorkStreamScriptPort):
  ...
  runtime.OBDC_NS = "http://ontobdc.org/ontology/domain/ns.ttl#";
...
"""
  ...
  const WORK_STREAM_RESOURCE_NAME = "work_stream";
  ...
  async function isContainerHandle(handle) {
    try {
      ...
      let datapackage;
      try {
        datapackage = JSON.parse(datapackageText);
      } catch (jsonError) {
        return false;
      }
  ...
  async function openContainer() {
    ...
    setConnectButtonLabel(t("requestingAccess"));
    ...
    try {
      ...
      // Step 2 — Pyodide + mount + parse. Uses ensurePyodide so we don't
      // re-download rdflib/openpyxl on every click.
      ...
      const userMessage = message.includes("datapackage.json")
        ...
        : message;
      const fallbackLabel = runtime.state.rawContainerHandle
        ? t("reconnectFolder")
        : t("connectFolder");
      setConnectError(userMessage, fallbackLabel);
    } finally {
...
"""
  ...
  const ONTOBDC_CONNECTION_STATUS_EVENT = "ontobdc:workstream-connection-status";
...
"""
  ...
  function ensureAnnotationRuntime() {
    ...
    const folder = runtime.state.selfNode ? datasetFolderName(runtime.state.selfNode["@id"]) : null;
